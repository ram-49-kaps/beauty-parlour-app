...
const Signup = () => {
  ...
  const validateField = (name, value, otherField = null) => {
    ...
    if (name === 'name') {
      ...
      if (!value) errors.confirmPassword = 'Confirm password is required';
      else if (value !== (otherField || formData.password)) errors.confirmPassword = 'Passwords do not match';
  ...
  const handleSubmit = async (e) => {
    ...
    const confirmPasswordErrors = validateField('confirmPassword', formData.confirmPassword, formData.password);
    
    setFieldErrors({ name: nameErrors.name, email: emailErrors.email, password: passwordErrors.password, confirmPassword: confirmPasswordErrors.confirmPassword });
    
    if (nameErrors.name || emailErrors.email || passwordErrors.password || confirmPasswordErrors.confirmPassword) {
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center pt-24 pb-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden font-sans ${isDark ? 'bg-stone-950 text-white' : 'bg-gray-50 text-gray-900'}`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input type="text" name="name" value={formData.name} onChange={handleChange} className={`${inputCls} ${fieldErrors.name ? (isDark ? 'border-red-500/50 bg-red-500/10' : 'border-red-300 bg-red-50') : ''}`} placeholder="ENTER YOUR NAME" required />
              </div>
              {fieldErrors.name && <p className="text-red-500 text-xs font-light mt-1 ml-1">{fieldErrors.name}</p>}
            </div>
            <div className="space-y-2">
              <label className={labelCls}>Email Address</label>
              <div className="relative group">
                <Mail className={iconCls} />
                <input type="email" name="email" value={formData.email} onChange={handleChange} className={`${inputCls} ${fieldErrors.email ? (isDark ? 'border-red-500/50 bg-red-500/10' : 'border-red-300 bg-red-50') : ''}`} placeholder="NAME@EXAMPLE.COM" required />
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input type={showPassword ? "text" : "password"} name="password" value={formData.password} onChange={handleChange} className={`${inputPwCls} ${fieldErrors.password ? (isDark ? 'border-red-500/50 bg-red-500/10' : 'border-red-300 bg-red-50') : ''}`} placeholder="••••••••" required />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className={`absolute right-4 top-1/2 -translate-y-1/2 ${isDark ? 'text-stone-500 hover:text-white' : 'text-gray-400 hover:text-gray-900'}`}>{showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button>
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <Lock className={iconCls} />
                <input type={showConfirmPassword ? "text" : "password"} name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} className={`${inputPwCls} ${fieldErrors.confirmPassword ? (isDark ? 'border-red-500/50 bg-red-500/10' : 'border-red-300 bg-red-50') : ''}`} placeholder="••••••••" required />
                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className={`absolute right-4 top-1/2 -translate-y-1/2 ${isDark ? 'text-stone-500 hover:text-white' : 'text-gray-400 hover:text-gray-900'}`}>{showConfirmPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button>
              </div>
              {fieldErrors.confirmPassword && <p className="text-red-500 text-xs font-light mt-1 ml-1">{fieldErrors.confirmPassword}</p>}
            </div>
